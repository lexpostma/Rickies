...
if ($all_event_details) {
	$required_event_fields = ["Name", "URL", "Winner (manual)", "Episode date", "Rickies artwork", "Event artwork", "Episode artwork", "Ricky winner"];
} else {
	$required_event_fields = ["Name", "URL", "Winner (manual)", "Episode date", "Rickies artwork", "Event artwork", "Episode artwork"];
}


if (isset($rickies_events_params) && !array_key_exists("fields", $rickies_events_params)) {
...
do {
	...
	// if(count($rickies_events_response['records']) > 0){
		...
		foreach($rickies_events_response[ 'records' ] as $array) {
			...
			$rickies_events_array[$id]["date_string"] = strftime("%e %B %Y", $rickies_events_array[$id]["date"]);
			...
			if ($all_event_details) {
				...
				$rickies_events_array[$id]["label3"] = $rickies_events_array[$id]["date_string"];
			}
			...
			foreach ($rickies_events_array[$id]["artwork"] as $source => $artwork) {
				if ($artwork !== false && !isset($rickies_events_array[$id]["img_url"])) {
